...
(function () {
    ...
    function SelectHandler(el) {
        ...
        this.pointCoords = function (point, object) {
            const settings = this.pointsList[point];
    ...
    SelectHandler.prototype.init = function (value, options) {
        ...
        const { points } = this.el.selectize.defaults;
        ...
        const pointsLists = ['points', 'pointsExclude'];
    ...
    SelectHandler.prototype.getPointArray = function () {
        ...
        return this.el.array().valueOf().map((el) => [el[0] - bbox.x, el[1] - bbox.y]);
    };
    ...
    SelectHandler.prototype.drawPoints = function () {
        ...
        for (let i = 0, len = array.length; i < len; ++i) {
            ...
            const point = this.drawPoint(array[i][0], array[i][1])
    ...
    SelectHandler.prototype.updatePointSelection = function () {
        const array = this.getPointArray();
    ...
    SelectHandler.prototype.updateRectSelection = function () {
        ...
        if (this.options.points.length) {
            this.options.points.map((point, index) => {
                const coords = _this.pointCoords(point, bbox);
        ...
        if (this.options.rotationPoint) {
            const length = this.rectSelection.set.length();
    ...
    SelectHandler.prototype.selectRect = function (value) {
        ...
        function getMoseDownFunc(eventName) {
            return function (ev) {
                ...
                const x = ev.pageX || ev.touches[0].pageX;
                const y = ev.pageY || ev.touches[0].pageY;
                _this.el.fire(eventName, { x, y, event: ev });
            };
        }
        ...
        if (this.options.points.length && this.rectSelection.set.length() < 2) {
            const ename = 'touchstart'; const
                mname = 'mousedown';

            this.options.points.map((point, index) => {
                const coords = _this.pointCoords(point, bbox);

                const pointElement = _this.drawPoint(coords.x, coords.y)
        ...
        if (this.options.rotationPoint && ((this.options.points && !this.rectSelection.set.get(9)) || (!this.options.points && !this.rectSelection.set.get(1)))) {
            const curriedEvent = function (ev) {
                ev = ev || window.event;
                ev.preventDefault ? ev.preventDefault() : ev.returnValue = false;
                ev.stopPropagation();
                ...
                const x = ev.pageX || ev.touches[0].pageX;
                const y = ev.pageY || ev.touches[0].pageY;
                _this.el.fire('rot', { x, y, event: ev });
            };

            const pointElement = this.drawPoint(bbox.width / 2, 20)
    ...
    SelectHandler.prototype.observe = function () {
        ...
        if (MutationObserver) {
            if (this.rectSelection.isSelected || this.pointSelection.isSelected) {
                this.observerInst = this.observerInst || new MutationObserver((() => {
                    _this.handler();
                }));
    ...
    // Select element with mouse
        ...
        // Check the parameters and reassign if needed
            ...
            const selectHandler = this.remember('_selectHandler') || new SelectHandler(this);
